...
export class ServerUtil<T extends AbstractClient<any, any, any>> extends Util<T> {
    ...
    async getMember(serverId: string, userId: string, cache = true, force = false): Promise<TeamMemberPayload> {
        if (!force) {
    ...
    // Cache this member
    setMember(serverId: string, userId: string, data: TeamMemberPayload) {
