...
export default {
    execute: async ([reaction, ctx]) => {
        ...
        if (createdBy !== ctx.user?.id && emote.id === defaultGiveawayEmote && ctx.giveawayUtil.participants[messageId]) {
            const giveaway = ctx.giveawayUtil.participants[messageId];

            // Remove the user from participating
            giveaway.users.splice(giveaway.users.indexOf(createdBy), 1);
        }
    },
