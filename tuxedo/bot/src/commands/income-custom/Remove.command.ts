...
const Remove: Command = {
    ...
    execute: async (message, args, ctx, { server }) => {
        ...
        if (incomeType) return disableDefaultIncome(message, ctx, server, incomeType);
        ...
        if (!incomeCommand) return ctx.messageUtil.replyWithError(message, "Doesn't exist", `The income by the name of ${inlineQuote(name)} does not exist.`);
...
async function disableDefaultIncome(message: Message, ctx: TuxoClient, server: Server, incomeType: DefaultIncomeType) {
    ...
    return ctx.messageUtil.replyWithSuccess(
        message,
        "Income disabled",
        `The income ${inlineQuote(incomeType.toLowerCase())} has been successfully disabled and can no longer be used.`
    );
}
