...
const Delete: Command = {
    ...
    execute: async (message, args, ctx) => {
        ...
        else if (currency.startingBalance === value)
            return ctx.messageUtil.replyWithError(
                message,
                "Already set",
                `The starting balance for the currency with tag ${inlineQuote(tag)} is already set to ${inlineCode(value.toString())}.`
            );
        // Doesn't make sense if we start > maximumBalance
        else if (currency.maximumBalance && currency.maximumBalance <= value)
            return ctx.messageUtil.replyWithError(
                message,
                "Invalid value",
                `The starting balance for the currency with tag ${inlineQuote(tag)} cannot be equal to or greater than the maximum balance, which is ${currency.maximumBalance}.`
            );
        ...
        return ctx.messageUtil.replyWithSuccess(
            message,
            "Currency starting balance set",
            `Currency with the tag ${inlineQuote(tag)} now has a starting balance of ${inlineQuote(value)}.`
        );
    },
};
