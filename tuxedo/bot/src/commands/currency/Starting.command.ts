...
const Delete: Command = {
    ...
    execute: async (message, args, ctx) => {
        ...
        // Check if the starting balance is already set to the value
        if (currency.startingBalance === value) return ctx.messageUtil.replyWithError(message, "Already set", `The starting balance for the currency with tag ${inlineQuote(tag)} is already set to ${inlineCode(value.toString())}.`);
        // Check if the starting balance is equal to or greater than the maximum balance
        if ((currency.maximumBalance) && (currency.maximumBalance <= value)) return ctx.messageUtil.replyWithError(message, "Invalid value", `The starting balance for the currency with tag ${inlineQuote(tag)} cannot be equal to or greater than the maximum balance, which is ${currency.maximumBalance}.`);

        await ctx.dbUtil.editCurrencyStartingBalance(currency, value);
