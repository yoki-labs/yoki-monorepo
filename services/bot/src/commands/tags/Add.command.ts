...
const Add: Command = {
    ...
    execute: async (message, args, ctx) => {
        ...
        if (tagName.length > 120) return ctx.messageUtil.replyWithAlert(message, `Content too long`, "Tag name is too long! Please shorten it to under 120 characters.");
        if (tagContent.length > 800) return ctx.messageUtil.replyWithAlert(message, `Name too long`, "Tag content is too long! Please shorten it to under 800 characters.");

        const doesAlreadyExist = await ctx.prisma.customTag.findFirst({ where: { serverId: message.serverId!, name: tagName } });
        if (doesAlreadyExist) return ctx.messageUtil.replyWithAlert(message, `Already exists`, "There is already a tag with that name.");
