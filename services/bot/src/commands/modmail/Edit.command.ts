...
const Edit: Command = {
    ...
    execute: async (message, args, ctx, { member }) => {
        ...
        if (!sentModmailMessage) return ctx.messageUtil.replyWithError(message, "That is not a valid sent modmail message!");
        ...
        if (!fetchSentModmailMessage) return ctx.messageUtil.replyWithError(message, "This message has been manually deleted.");
