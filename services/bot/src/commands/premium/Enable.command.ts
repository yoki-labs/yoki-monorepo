...
const Enable: Command = {
    ...
    execute: async (message, args, ctx) => {
        ...
        if (!tier) return ctx.messageUtil.replyWithError(message, "That tier type doesn't exist!");
        const server = await ctx.prisma.server.findFirst({ where: { serverId } });
        if (!server) return ctx.messageUtil.replyWithError(message, "That server does not exist!");
