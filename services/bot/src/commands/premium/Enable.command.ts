...
const Enable: Command = {
    ...
    execute: async (message, args, ctx) => {
        ...
        if (!tier) return ctx.messageUtil.replyWithAlert(message, `No tier`, "That tier type doesn't exist!");
        const server = await ctx.prisma.server.findFirst({ where: { serverId } });
        if (!server) return ctx.messageUtil.replyWithAlert(message, `No server`, "That server does not exist!");
