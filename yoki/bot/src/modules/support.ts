import { Util, inlineCode } from "@yokilabs/bot";
...
export default class SupportUtil extends Util<YokiClient> {
    async createModmailThread(
        ...
        const modmailMessage = await this.messageUtil.sendInfoBlock(
            channelId,
            `New modmail ticket`,
    ...
    async closeExistingThread(
        server: Server,
        ticket: ModmailThread,
        closedBy: string,
    ) {
        // Since they are sort of contributing to it
        const handlingModerators =
            ticket.handlingModerators.includes(closedBy)
            ? ticket.handlingModerators
            : ticket.handlingModerators.concat(closedBy);
    ...
    async sendModmailCloseMessage(
        ...
        if (!(modmailReaction?.channelId || modmailLogChannel))
            return;
        ...
        if (modmailLogChannel)
            await this.sendModmailLog(serverId, ticket, modmailLogChannel, closedState, historyMessage);
    }

    async sendModmailLog(
        serverId: string,
        ticket: ModmailThread,
        modmailLogChannel: LogChannel,
        closedState: string,
