...
const LinkSeverity: Command = {
	...
	execute: async (message, args, ctx, { server }) => {
		...
		return ctx.messageUtil.replyWithSuccess(
			...
			`Non-whitelisted URLs or invites will now give ${inlineCode(infractions ?? server.linkInfractionPoints)} infraction points and a ${inlineCode(severity ?? server.linkSeverity)}.`
		);
	},
};
