...
const Member: Command = {
    ...
    execute: async (message, args, ctx, commandCtx) => {
        ...
        if (Number.isNaN(roleId))
            return ctx.messageUtil.replyWithError(message, `Provide role ID`, `Provide a valid ID of the role you want to set as a member role or pass \`remove\` to remove it.`);
