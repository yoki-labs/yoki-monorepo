						...
						[
							...
							},
							{
								...
								value: stripIndents`
                                    ...
                                    ${
																			currentRole.colors
																				? `**Colors:** ${currentRole.colors.map(hexColor).join(" - ")}`
																				: "**No color.**"
																		}
                                    **Role created:** ${server.formatTimezone(new Date(currentRole.createdAt))} EST
                                    ${
																			currentRole.updatedAt &&
																			`**Last updated:** ${server.formatTimezone(new Date(currentRole.updatedAt))} EST`
																		}
                                `,
							},
...
const displayRoleList = ({ isMentionable, isDisplayedSeparately, isSelfAssignable, permissions }: Role) =>
	`${isMentionable || isDisplayedSeparately || isSelfAssignable ? "... And " : ""}**${
		permissions.length
	}** permissions.`;
