import { ChannelRolePermission, ChannelUserPermission, Permission, channelName, channelTypeToDisplay, channelTypeToGreyIcon, inlineCode, inlineQuote } from "@yokilabs/bot";
...
const ChannelInfo: Command = {
    ...
    execute: async (message, args, ctx, { server, member }) => {
        ...
        if (message.groupId !== channel.groupId)
            return ctx.messageUtil.replyWithError(message, "Wrong group", `As of now, Yoki cannot check the group membership status of particular users or roles. As such, you can only get info of channels within the same group.`);
                    ...
                    .addFields(
                        [
                            ...
                            },
                            {
                                ...
                                value: stripIndents`
                                    **Channel ID:** ${inlineCode(channel.id)}${channel.archivedAt ? `\n:file_cabinet: **Is archived.**` : ""}${(channel.raw as { visibility?: "public" }).visibility === "public" ? `\n:earth_americas: **Is public.**\n` : ""}
...
async function memberHasRoleChannelPermissions(ctx: YokiClient, channel: Channel, member: Member) {
    ...
    const serverRoles = await ctx.roles
        .fetchMany(channel.serverId);
