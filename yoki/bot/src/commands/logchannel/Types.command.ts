...
const categorizedLogs: Collection<string, LogChannelType[]> =
    new Collection<string, LogChannelType[]>()
        .set("Content", [
        ...
        .set("Members", [
            LogChannelType.member_joins,
            LogChannelType.member_leaves,
            LogChannelType.member_bans,
            LogChannelType.member_updates,
            LogChannelType.member_roles_updates,
        ])
...
const Types: Command = {
    ...
    execute: (message, _args, ctx, { prefix }) => {
        return ctx.messageUtil.replyWithCategorizedList(message, `Log channel types`, categorizedLogs, `To add a log channel, type \`${prefix}logs set [channel here] [log type here]\``);
    },
};
