...
const Channel: Command = {
    ...
    execute: async (message, args, ctx, commandCtx) => {
        ...
        if (!channel) {
            const fetchedChannel = commandCtx.server.antiRaidChallengeChannel
                ? await ctx.channels.fetch(commandCtx.server.antiRaidChallengeChannel).catch(() => commandCtx.server.antiRaidChallengeChannel)
                : null;
