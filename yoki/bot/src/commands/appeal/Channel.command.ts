...
const Channel: Command = {
	...
	execute: async (message, args, ctx, commandCtx) => {
		...
		if (!channel) {
			const fetchedChannel = commandCtx.server.appealChannel ? await ctx.rest.router.getChannel(commandCtx.server.appealChannel).then(x => x.channel).catch(() => commandCtx.server.appealChannel) : null;
