...
const Ignore: Command = {
  ...
  args: [{ name: "channel", type: "channel", optional: true }, { name: "action", type: "string", optional: true }],
  execute: async (message, args, ctx) => {
    ...
    if (!channel) {
      ...
      return ctx.messageUtil.replyWithInfo(message, "Ignored channels by ID", listInlineCode(dbChannels.map(x => x.channelId)));
    }
    ...
    if (action?.toLowerCase().trim() === "remove") {
      ...
      if (existingAlready) return ctx.messageUtil.replyWithError(message, "The bot is already ignoring this channel", "You have already marked this channel to be ignored by the automod filter. If you wish for it to not be ignored, you can run this command again with \"remove\" at the end of it.")
    ...
    return ctx.messageUtil.replyWithSuccess(message, "Successfully marked channel as ignored", `The bot will no longer scan messages in this channel. You can undo this by running the command again with \"remove\" at the end of it.`);
  },
};
